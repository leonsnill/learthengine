...
source activate RS
cd '/Users/leonnill/Google Drive/03_LSNRS/01_python/PycharmProjects/EarthEngine'
...
SENSOR = 'SL8'
CLOUD_COVER = 60
BANDS = ['R', 'NIR', 'SWIR2']  # 'B', 'G', 'R', 'NIR', 'SWIR1', 'SWIR2'  # 'TCB', 'TCG', 'TCW'
PIXEL_RESOLUTION = 30
...
if MONTHLY:
    TARGET_MONTHS_client = [4, 5, 6, 7, 8, 9, 10]
    MONTHS_OFFSET = 0
    STMs = [ee.Reducer.median()]
else:
    TARGET_DOY_client = [175, 258]
...
def fun_mask_ls_sr(img):
    ...
    return img.addBands(cloud).updateMask(mask).divide(10000).float() \
    ...
    if MONTHLY:
        ...
        for month in TARGET_MONTHS_client:
            ...
            if months_max - months_min == 0:
                out_file = SENSOR + '_STMs_' + ROI_NAME + '_' + str(
                    PIXEL_RESOLUTION) + 'm_' + year_filename + '_' + str(month)
            elif months_max - months_min == 11:
                out_file = SENSOR + '_STMs_' + ROI_NAME + '_' + str(PIXEL_RESOLUTION) + 'm_' + year_filename
            else:
                out_file = SENSOR + '_STMs_' + ROI_NAME + '_' + str(
                    PIXEL_RESOLUTION) + 'm_' + year_filename + '_' + str(months_min) + '-' \
                           + str(months_max)
        ...
        for i in range(len(TARGET_DOY_client)):
            ...
            out_file = SENSOR + '_imgComposite_' + SCORE + '_' + ROI_NAME + '_' + str(
                PIXEL_RESOLUTION) + 'm_' + year_filename + '_' + str(iter_target_doy)

            out = ee.batch.Export.image.toDrive(image=COMPOSITE, description=out_file,
                                                scale=PIXEL_RESOLUTION,
                                                maxPixels=1e13,
                                                region=ROI['coordinates'][0],
                                                crs=EPSG)
